...
export function Customers() {
  ...
  const { data: customerDetail, isLoading: detailLoading } = useGetCustomer(activeCustomerId as number, { query: { enabled: !!activeCustomerId, queryKey: getGetCustomerQueryKey(activeCustomerId as number) }});
  ...
  const handlePayment = () => {
    ...
    recordPayment.mutate({
      ...
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: getListCustomersQueryKey() });
        queryClient.invalidateQueries({ queryKey: getGetCustomerQueryKey(activeCustomerId) });
      }
    });
  };
            ...
            <div className="flex-1 overflow-auto p-6">
              ...
              {customerDetail.ledger.length === 0 ? (
                ...
                <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
                  {customerDetail.ledger.map((entry, idx) => (
