            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-4">
              {filteredProducts.map(product => (
                <button
                  ...
                  className="flex flex-col items-start p-4 text-left border border-border/50 rounded-lg hover:border-primary hover:shadow-md transition-all active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100 disabled:hover:border-border/50 group bg-background"
                >
                  <div className="w-full flex justify-between items-start mb-2">
                    <Badge variant="secondary" className="text-[10px] px-1.5">{product.categoryName}</Badge>
                    {product.stockLevel <= 0 ? (
                      <Badge variant="destructive" className="text-[10px] px-1.5">Out of Stock</Badge>
                    ) : product.isLowStock ? (
                      <Badge variant="outline" className="text-[10px] px-1.5 text-orange-500 border-orange-200 bg-orange-50 dark:bg-orange-950/30">Low Stock: {product.stockLevel}</Badge>
                    ) : (
                      <span className="text-xs font-mono text-muted-foreground">{product.stockLevel} in stock</span>
                    )}
                  </div>
                  <span className="font-semibold leading-tight line-clamp-2 mb-1 group-hover:text-primary transition-colors">{product.name}</span>
                  <span className="mt-auto font-mono font-bold text-lg">{formatMoney(product.sellingPriceCents)}</span>
                </button>
              ))}
            </div>
          )}
...
// A simple package icon fallback for the missing import
function Package(props: any) {
  return <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinelinejoin="round" {...props}><path d="M16.5 9.4 7.5 4.21"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>;
}
